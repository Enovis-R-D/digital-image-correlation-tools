...
class InterfaceEventHandler(PySpin.InterfaceEventHandler):
    ...
    and OnDeviceRemoval() methods. Also, enumeration event handlers must inherit from
...
class SystemEventHandler(PySpin.InterfaceEventHandler):
    """
    In the C++ example, the SystemEventHandler inherits from both DeviceArrivalEventHandler and
    DeviceRemovalEventHandler. This doesn't work for this wrapper, as it will only inherit the abstract
    method from the first base class listed, so for this example both System and Interface
    event handlers inherit from InterfaceEventHandler.
    All three event handler types - DeviceArrivalEventHandler, DeviceRemovalEventHandler, and InterfaceEventHandler - can be
...
def main():
    ...
    for i, iface in enumerate(iface_list):
        ...
        iface.RegisterEventHandler(interface_events[i])
    ...
    for i, iface in enumerate(iface_list):
        iface.UnregisterEventHandler(interface_events[i])

    # Release reference to interface and interface event handlers
    del iface
